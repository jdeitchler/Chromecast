...
CastPlayer.prototype.loadMedia = function(mediaIndex) {
  ...
  var v = this.mediaContents[mediaIndex];
  var mediaInfo = new chrome.cast.media.MediaInfo(v.id);

  mediaInfo.metadata = new chrome.cast.media.GenericMediaMetadata();
  mediaInfo.metadata.metadataType = chrome.cast.media.MetadataType.GENERIC;
  mediaInfo.contentType = 'video/mp4';

  mediaInfo.metadata.title = v.title;
  mediaInfo.metadata.images = [{'url': v.imagePath}];
...
CastPlayer.prototype.playMediaLocally = function() {
  ...
  console.log(this.mediaContents[this.currentMediaIndex], this.mediaContents[this.currentMediaIndex]['iOS_streamURL'])
  if( this.localPlayerState != PLAYER_STATE.PLAYING && this.localPlayerState != PLAYER_STATE.PAUSED ) { 
    this.localPlayer.src = this.mediaContents[this.currentMediaIndex]['iOS_streamURL'];
...
CastPlayer.prototype.selectMediaUpdateUI = function(mediaIndex) {
  var v = this.mediaContents[mediaIndex];
  document.getElementById('video_image').src = v.thumbnailPath;
  document.getElementById("progress").style.width = '0px';
  document.getElementById("media_title").innerHTML = v.title;
  document.getElementById("media_subtitle").innerHTML = "";
  document.getElementById("media_desc").innerHTML = v.description;
};
 ...
 */
CastPlayer.prototype.retrieveMediaJSON = function() {
...
CastPlayer.prototype.onMediaJsonLoad = function(evt) {
  ...
  var divIdName = null;
  for( var i = 0; i < this.mediaContents.length; i++ ) {
    var elem = this.mediaContents[i];
    newdiv = document.createElement('div');
    divIdName = 'thumb'+i+'Div';
    newdiv.setAttribute('id',divIdName);
    newdiv.setAttribute('class','thumb');
    newdiv.innerHTML = '<img src="' + elem.thumbnailPath + '" class="thumbnail">';
