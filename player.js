...
sampleplayer.CastPlayer = function(element) {
   ...
   * @private {boolean}
   */
  this.debug_ = true;// sampleplayer.DISABLE_DEBUG_;
...
sampleplayer.CastPlayer.prototype.preloadVideo_ = function(mediaInformation) {
  ...
  //this.log_("sampleplayer.CastPlayer.prototype.preloadVideo_" + url + " " + type + " " + path);
...
sampleplayer.CastPlayer.prototype.load = function(info) {
  ...
  xhr.addEventListener('load', function(evt) {
  	  self.log_('onMediaJsonLoad' + location.host);
	  var video = {};
      //var xhr = e.target;
      //var res = xhr.getResponseJson();
      var res = JSON.parse(evt.srcElement.response);
      var video = {};
	  try
	  {
		  var res = JSON.parse(evt.srcElement.response);
		  self.log_(evt.response);
		  if (res != null && res.length > 0){
		  	 var temp = res[0].videos;
		   	 for (var pName in temp){
		   	   video[temp[pName].id] = temp[pName];
		   	 }
		   }
		   var id = video[media.contentId].iOS_streamURL;
		   media.contentId = id;
		   self.log_(id);
	   }
	   catch(e)
  ...
  xhr.open('GET', "https://www.lifetimefitness.com/ltfvideo/fitness");
  xhr.send(null);
	  /*
	  var xhr = new XMLHttpRequest();
	  xhr.addEventListener('load', function(evt) {
	  ...
	  xhr.open('GET', "https://www.lifetimefitness.com/ltfvideo/fitness", false);
	  xhr.send(null);
...
sampleplayer.CastPlayer.prototype.loadMetadata_ = function(media) {
  this.log_('loadMetadata_' + JSON.stringify(media));
  if (!sampleplayer.isCastForAudioDevice_()) {
    var metadata = media.metadata || {};
    var titleElement = this.element_.querySelector('.media-title');

    if (!this.debug_)
    	sampleplayer.setInnerText_(titleElement, metadata.title + " " + media.contentId);
...
sampleplayer.CastPlayer.prototype.loadPreviewModeMetadata_ = function(media) {
  ...
  if (!sampleplayer.isCastForAudioDevice_()) {
    ...
    sampleplayer.setInnerText_(titleElement, metadata.title + metadata.contentId);
...
sampleplayer.CastPlayer.prototype.loadVideo_ = function(info) {
  this.log_('loadVideo_' + info.message.media.contentId);
  var self = this;
  ...
  this.log_('loadVideo_: ' + type + " " + path + " " + info.message.media.contentType);
  ...
  if (!protocolFunc) {
    ...
    var loadErrorCallback = function(ev) {
      // unload player and trigger error event on media element
      self.log_('loadErrorCallback: ' + ev);
      if (self.player_) {
        self.resetMediaElement_();
...
sampleplayer.CastPlayer.prototype.onLoadMetadataError_ = function(event) {
  this.log_('onLoadMetadataError_'  + JSON.stringify(event));
  var self = this;
  sampleplayer.transition_(self.element_, sampleplayer.TRANSITION_DURATION_,
      function() {
        self.setState_(sampleplayer.State.IDLE, true);
...
sampleplayer.getProtocolFunction_ = function(mediaInformation) {
  ...
  if (sampleplayer.getExtension_(path) === 'm3u8' ||
    ...
    mediaInformation.contentType = 'application/vnd.apple.mpegurl';
    ...
    mediaInformation.contentType = 'application/vnd.ms-sstr+xml';
...
sampleplayer.CastPlayer.prototype.log_ = function(message) {
  if (this.debug_ && message) {
    var elem = this.element_.querySelector('.media-title');
    elem.innerHTML = elem.innerHTML + "<br/>" + message;
    elem.scrollTop = 9999999;
  	//sampleplayer.setInnerText_(elem, elem.innerHTML + "<br/>" + message);
    console.log(message);
  }
};
